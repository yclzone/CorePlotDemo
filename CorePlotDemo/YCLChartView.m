...
static CGFloat const TEMP_MAX = 42.0f+0.5f;
static CGFloat const TEMP_MIN = 32.0f-0.5f;
static CGFloat const TEMP_INTERVAL = 1.0f;

static NSString *const CPDTickerSymbolAAPL = @"Plot";

@interface YCLChartView ()<CPTPlotDataSource, CPTPlotSpaceDelegate,
                           CPTAxisDelegate>
/** hostingView */
...
- (void)configureGraph {
  ...
  graph.fill =
      [CPTFill fillWithColor:[[CPTColor grayColor] colorWithAlphaComponent:1]];

  graph.paddingTop = 0;
  graph.paddingLeft = 0;
  graph.paddingBottom = 00;
  ...
  plotAreaFrame.fill = [CPTFill
      fillWithColor:[[CPTColor blackColor] colorWithAlphaComponent:0.5]];

  CPTPlotArea *plotArea = plotAreaFrame.plotArea;
  plotArea.fill =
      [CPTFill fillWithColor:[[CPTColor redColor] colorWithAlphaComponent:0.2]];
...
- (void)configurePlots {
  ...
  plotSpace.xRange =
      [CPTPlotRange plotRangeWithLocation:@0.0 length:@(MINUTES_ONE_DAY)];
  ...
  plotSpace.globalYRange =
      [CPTPlotRange plotRangeWithLocation:@(TEMP_MIN) length:@(TEMP_MAX-TEMP_MIN)];
  plotSpace.yRange = [CPTPlotRange plotRangeWithLocation:@(TEMP_MIN) length:@(TEMP_MAX-TEMP_MIN)];
...
- (void)configureAxes {
  ...
  x.axisConstraints =
      [CPTConstraints constraintWithLowerOffset:0];  //固定坐标轴
  ...
  //    CGFloat dateCount = [[[CPDStockPriceStore sharedInstance] datesInMonth]
  //    count];
  //    NSMutableSet *xLabels = [NSMutableSet setWithCapacity:dateCount];
  //    NSMutableSet *xLocations = [NSMutableSet setWithCapacity:dateCount];
  //    NSInteger i = 0;
  //    for (NSString *date in [[CPDStockPriceStore sharedInstance]
  //    datesInMonth]) {
  //        CPTAxisLabel *label = [[CPTAxisLabel alloc] initWithText:date
  ...
  y.majorIntervalLength = @(TEMP_INTERVAL);
  ...
  //    CGFloat yMax = 700.0f;  // should determine dynamically based on max
  //    price
  ...
  //            initWithText:[NSString stringWithFormat:@"%i", j]
  ...
  //            }
  //            [yMajorLocations addObject:[NSDecimalNumber
  //            decimalNumberWithDecimal:location]];
  //        } else {
  //            [yMinorLocations addObject:[NSDecimalNumber
  //            decimalNumberWithDecimal:CPTDecimalFromInteger(j)]];
  //        }
  //    }
...
- (BOOL)axis:(CPTAxis *)axis
  ...
  for (NSDecimalNumber *tickLocation in locations) {
    ...
    if ([axis.identifier isEqual:@"x"]) {
      ...
      NSInteger time =
          [NSDate hy_timeintervalFromDate:date type:HYTimeintervalTypeUpToDay];
      NSInteger aTime = tickLocation.integerValue * 60 + time;
      labelString =
          [NSString hy_stringFromTimeinterval:aTime withFormat:@"HH:mm"];
    }

    CPTTextLayer *newLabelLayer =
        [[CPTTextLayer alloc] initWithText:labelString
                                     style:[axis.labelTextStyle mutableCopy]];
...
- (CPTPlotRange *)plotSpace:(CPTPlotSpace *)space
  ...
  if (coordinate == CPTCoordinateY) {
    return [CPTPlotRange plotRangeWithLocation:@(TEMP_MIN) length:@(TEMP_MAX-TEMP_MIN)];
    ;
  } else {
    ...
    } else if (newRange.lengthDouble >=
               MINUTES_ONE_HOUR * 6) {  // 24个点，间隔1刻钟
      xAxis.majorIntervalLength = @(MINUTES_ONE_QUARTER * 3);
      xAxis.minorTicksPerInterval = 0;
    } else if (newRange.lengthDouble >
               MINUTES_ONE_HOUR * 4) {  // 24个点，间隔10分钟
      xAxis.majorIntervalLength = @(10 * 3);
      xAxis.minorTicksPerInterval = 0;
    } else if (newRange.lengthDouble >
               MINUTES_ONE_HOUR * 2) {  // 24个点，间隔5分钟
      xAxis.majorIntervalLength = @(5 * 3);
      xAxis.minorTicksPerInterval = 0;
    } else if (newRange.lengthDouble >
               MINUTES_ONE_HOUR * 1) {  // 20个点，间隔3分钟
      xAxis.majorIntervalLength = @(3 * 3);
      xAxis.minorTicksPerInterval = 2;
    } else if (newRange.lengthDouble >
               MINUTES_ONE_QUARTER * 2) {  // 15个点，间隔1分钟
